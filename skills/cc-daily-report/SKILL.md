...
以下のコマンドでセッションデータを取得する:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/parse_sessions.py [日付]
```
...
`${CLAUDE_PLUGIN_ROOT}/config.yml` でカスタマイズ可能。
設定例は `config.yml.example`、各項目の詳細は `README.md` を参照。
