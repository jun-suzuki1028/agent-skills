...
- **[参考]**: 未利用だが有益そうな機能・パターンの情報提供（導入を強く勧めない）
...
- 利用中の機能に対しては [GAP] / [推奨] で提案する
- 未利用だが有益な機能・パターンは [参考] として軽く情報提供する（導入を強く勧めない）
...
3. **改善機会** — 利用中の機能で、ベスプラに紹介されているが未活用のパターン
4. **参考情報** — 未利用だが、プロジェクトに有益そうな機能・パターンの紹介（導入を強く勧めない）

### フィルタリング原則

- **利用中の機能** → [GAP] / [推奨] で具体的に提案する
- **未利用だが有益な機能** → [参考] として軽く情報提供する（例: 「Hooksを活用するとXが自動化できます」）
- 未利用かつ関連性の薄い機能は省略する（例: MCP未使用かつ外部サービス連携もないプロジェクトへの.mcp.json提案）
